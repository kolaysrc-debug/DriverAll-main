// PATH: DriverAll-main/drivercv-backend/services/otp/providers/twilioVerify.js
...
async function resolveTwilioCreds(credentialRef) {
  ...
  let fromDb = null;
  try {
    fromDb = await otpCreds.getCredential("twilioVerify", ref);
  } catch (e) {
    dbErr = e;
  }

  if (fromDb?.accountSid && fromDb?.authToken && fromDb?.verifyServiceSid) return fromDb;
  ...
  if (!accountSid || !authToken) {
    if (dbErr) {
      const msg =
        "Twilio credentials exist in DB but cannot be decrypted. " +
        "OTP_SECRETS_MASTER_KEY likely changed; re-save credentials via /api/admin/otp-credentials.";
      const err = new Error(msg);
      err.code = "OTP_CREDENTIAL_DECRYPT_FAILED";
      err.cause = dbErr;
      throw err;
    }
...
async function checkOtp({ to, code, credentialRef }) {
  ...
  return {
    status: res.status, // approved | pending | canceled
    raw: res,
  };
}
