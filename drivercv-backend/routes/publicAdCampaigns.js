...
router.get("/slot", async (req, res) => {
  try {
    ...
    // 1 sabit + N kayar
    const fixed = filtered[0] || null;

    const maxItems = Number(placement?.carouselMaxItems || 10);
    const speedMs = Number(placement?.carouselSpeedMs || 4500);
    const carousel = filtered.slice(1, 1 + maxItems);
    ...
    return res.json({
      ...
      fixed: fixed
        ? {
            _id: fixed._id,
            title: fixed.title || "",
            clickUrl: fixed.clickUrl || "",
            creatives: fixed.creatives || [],
            startAt: fixed.startAt || null,
            endAt: fixed.endAt || null,
          }
        : null,
      ...
      rules: {
        // ✅ kararın: kayarda max 2
        carouselMaxMergeUnits: Number(placement?.carouselMaxMergeUnits || 2),
        defaultCarouselSpeedMs: speedMs,
        defaultCarouselMaxItems: maxItems,
      },
    });
  } catch (err) {
