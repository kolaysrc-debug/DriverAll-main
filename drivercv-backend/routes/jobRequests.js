...
async function createJobRequestCore(req, res, forcedJobId) {
  try {
    ...
    const pkg = await JobPackage.findById(packageId).lean();
    ...
    if (!found) {
      return res.status(400).json({ success: false, message: "placement not allowed for this package" });
    }
    ...
    const geoLevel = String(body.geoLevel || pkg.geoLevel || "country").trim();
...
// body: { jobId, packageId, placementKey, requestedDays, note?, countryTargets?, geoLevel? }
// ----------------------------------------------------------
...
// body: { packageId, placementKey, requestedDays, ... }
// ----------------------------------------------------------
...
router.post("/:id/approve", requireAuth, requireRoles("admin"), async (req, res) => {
  try {
    ...
    job.packageId = doc.packageId || null;
    job.packageName = String(doc.packageName || "");
