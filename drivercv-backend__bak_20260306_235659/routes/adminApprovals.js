...
router.get("/advertisers", requireAuth, attachUser, requireAdmin, async (req, res) => {
  try {
    console.log("[adminApprovals] GET /advertisers", { status: req.query?.status, userId: req.userId });
    ...
    return res.status(500).json({
      success: false,
      message: "Onay listesi alınamadı.",
      error: err?.message || String(err),
    });
  }
});
...
router.patch(
  ...
  async (req, res) => {
    try {
      console.log("[adminApprovals] PATCH /advertisers/:id/approve", {
        id: req.params?.id,
        userId: req.userId,
      });
      ...
      u.approvedByUserId = req.user?._id || req.userId || null;
...
router.patch(
  ...
  async (req, res) => {
    try {
      console.log("[adminApprovals] PATCH /advertisers/:id/reject", {
        id: req.params?.id,
        userId: req.userId,
      });
      const id = req.params.id;
