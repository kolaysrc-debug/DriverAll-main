...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Güvenlik/ürün kuralı: sadece yayınlanmış ilanlara başvuru
      if (String(job.status || "") !== "published") {
        return res.status(400).json({ success: false, message: "job is not published" });
      }

      // Opsiyonel: startAt/endAt varsa yayın süresi dışındaki ilana başvuru engelle
      const startAtMs = job.startAt ? new Date(job.startAt).getTime() : null;
      const endAtMs = job.endAt ? new Date(job.endAt).getTime() : null;
      const now = Date.now();
      if (startAtMs && Number.isFinite(startAtMs) && now < startAtMs) {
        return res.status(400).json({ success: false, message: "job is not active yet" });
      }
      if (endAtMs && Number.isFinite(endAtMs) && now > endAtMs) {
        return res.status(400).json({ success: false, message: "job is expired" });
      }
