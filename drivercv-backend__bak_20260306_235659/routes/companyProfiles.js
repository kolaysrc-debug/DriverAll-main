...
// Admin: update ad targeting override (per company exception)
router.post("/admin/:id/ad-targeting-override", requireAuth, requireRoles("admin"), async (req, res) => {
  try {
    const id = req.params.id;
    const doc = await CompanyProfile.findById(id);
    if (!doc) return res.status(404).json({ success: false, message: "not found" });
    ...
    const geoLevel = String(b.geoLevel || "").trim();
    const geoTargets = Array.isArray(b.geoTargets) ? b.geoTargets.map((x) => String(x).trim()).filter(Boolean) : [];

    if (enabled) {
      const allowed = new Set(["country", "province", "district", "geoGroup"]);
      if (!allowed.has(geoLevel)) {
        return res.status(400).json({ success: false, message: "geoLevel invalid" });
      }
    }

    doc.adTargetingOverride = {
      enabled,
      geoLevel: enabled ? geoLevel : "",
      geoTargets: enabled ? geoTargets : [],
      note: String(b.note || ""),
    ...
    return res
      .status(500)
      .json({ success: false, message: "ad-targeting-override failed", error: err.message });
  }
});
