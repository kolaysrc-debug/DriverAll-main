...
router.get("/:id", requireAuth, requireRoles("admin"), async (req, res) => {
  try {
    ...
    if (!doc || doc.deletedAt) return res.status(404).json({ success: false, message: "not found" });
    return res.json({ success: true, package: doc });
  } catch (err) {
    return res.status(500).json({ success: false, message: "get package failed", error: err.message });
  }
});
...
router.post("/", requireAuth, requireRoles("admin"), async (req, res) => {
  try {
    ...
    const productKey = String(b.productKey || "").trim();
    const purchaseType = String(b.purchaseType || "").trim() || "one_time";
    const billingCycleMonthsRaw = b.billingCycleMonths;
    const billingCycleMonths =
      billingCycleMonthsRaw == null || billingCycleMonthsRaw === ""
        ? null
        : Number(billingCycleMonthsRaw);
    ...
    const doc = await Package.create({
      ...
      productKey,
      purchaseType,
      billingCycleMonths: Number.isFinite(billingCycleMonths) ? billingCycleMonths : null,
      name,
      code,
      ...
      credits: {
        ...
        jobPostCount: Number(b?.credits?.jobPostCount || 0),
        cvViewCount: Number(b?.credits?.cvViewCount || 0),
        cvSaveCount: Number(b?.credits?.cvSaveCount || 0),
      },
      ...
      rules: {
        ...
        listingDays: Number(b?.rules?.listingDays || 0) || 0,
        homeDays: Number(b?.rules?.homeDays || 0) || 0,
      },
...
router.put("/:id", requireAuth, requireRoles("admin"), async (req, res) => {
  try {
    ...
    if (b.productKey != null) p.productKey = String(b.productKey || "").trim();
    if (b.purchaseType != null) {
      const pt = String(b.purchaseType || "").trim();
      p.purchaseType = pt === "subscription" ? "subscription" : "one_time";
    }
    if (b.billingCycleMonths != null) {
      const bcm = b.billingCycleMonths === "" ? null : Number(b.billingCycleMonths);
      p.billingCycleMonths = Number.isFinite(bcm) ? bcm : null;
    }
    ...
    if (b.credits) {
      ...
      p.credits.jobPostCount = Number(b?.credits?.jobPostCount ?? p.credits.jobPostCount);
      p.credits.cvViewCount = Number(b?.credits?.cvViewCount ?? p.credits.cvViewCount);
      p.credits.cvSaveCount = Number(b?.credits?.cvSaveCount ?? p.credits.cvSaveCount);
    }
    ...
    if (b.rules) {
      ...
      if (b?.rules?.listingDays != null) p.rules.listingDays = Number(b.rules.listingDays || 0) || 0;
      if (b?.rules?.homeDays != null) p.rules.homeDays = Number(b.rules.homeDays || 0) || 0;
    }

    if (b.config != null) {
      p.config = b.config && typeof b.config === "object" ? b.config : {};
    }
