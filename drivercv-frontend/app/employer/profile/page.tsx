    ...
    <RoleGate allowRoles={["employer", "admin"]}>
      <div className="mx-auto max-w-5xl px-4 py-6 md:px-8">
        <div className="mb-4 flex items-end justify-between gap-3">
          <div>
            <h1 className="text-xl font-bold text-slate-50">Firma Profili</h1>
            <div className="text-xs text-slate-400">
              Lokasyon standardı tüm sistemde aynı: TR için il/ilçe listeden seçilir.
            </div>
          </div>
          ...
          <form onSubmit={onSave} className="grid gap-4 md:grid-cols-12">
            <div className="md:col-span-7 rounded-xl border border-slate-800 bg-slate-950 p-4">
              ...
              <div className="mt-3 grid gap-3">
                <div>
                  <label className="text-xs text-slate-400">Firma adı (ilanlarda görünen)</label>
                  <input
                    value={companyName}
                    ...
                    placeholder="Örn: ADRTÜRK Danışmanlık"
                  />
                </div>

                <div className="grid gap-3 md:grid-cols-2">
                  <div>
                    <label className="text-xs text-slate-400">Marka / Ticari ad</label>
                    <input
                      value={brandName}
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div>
                    <label className="text-xs text-slate-400">Sektör</label>
                    <input
                      value={sector}
                ...
                <div>
                  <label className="text-xs text-slate-400">Adres (opsiyonel)</label>
                  <textarea
                    value={address}
                    onChange={(e) => setAddress(e.target.value)}
                    className="mt-1 w-full rounded-lg border border-slate-800 bg-slate-950 px-3 py-2 text-sm text-slate-100 outline-none"
                    rows={2}
                    placeholder="İleride detaylandıracağız"
                  />
                </div>

                <div>
                  <label className="text-xs text-slate-400">Firma açıklaması</label>
                  <textarea
                    value={aboutCompany}
