        ...
        <section className="bg-slate-900/70 border border-slate-800 rounded-xl p-4 space-y-4">
          ...
          <form className="space-y-3" onSubmit={handleCreate}>
            <div className="grid md:grid-cols-2 gap-3">
              <input
                type="text"
                placeholder="Kriter Adı (ör: SRC1 Belgesi)"
                value={newLabel}
            ...
            <div className="grid md:grid-cols-4 gap-3 items-center">
              ...
              <input
                ...
                min={0}
                max={100}
                placeholder="Kalite Puanı (1–100)"
            ...
            <div className="grid md:grid-cols-3 gap-3 items-center">
              <select
                value={newOptionsGroupKey}
                onChange={(e) => setNewOptionsGroupKey(e.target.value)}
                className="rounded-lg bg-slate-950 border border-slate-700 px-3 py-1.5 text-xs outline-none"
              >
                <option value="">Seçenek grubu yok (free text)</option>
                {groups.map((g) => (
                  <option key={g._id} value={g.groupKey}>
                    {g.groupLabel} ({g.groupKey})
                  </option>
                ))}
              </select>

              <span className="text-[11px] text-slate-400 md:col-span-2">
                Eğer bu kriterin seçenekleri (örn. "Ehliyet Sınıfı") grup
                motorundan gelecekse buradan ilgili grubu seç.
              </span>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-3 items-center">
              <select
                value={newExpiryMode}
                onChange={(e) =>
                  setNewExpiryMode(e.target.value as ExpiryMode)
                }
                className="rounded-lg bg-slate-950 border border-slate-700 px-3 py-1.5 text-xs outline-none"
              >
                <option value="none">Süre / yaş kuralı yok</option>
                <option value="age">Yaşa bağlı biter (örn. 65)</option>
                <option value="durationFromIssue">
                  Alındığı tarihten itibaren X yıl
                </option>
              </select>
              ...
              <input
                type="number"
                min={0}
                placeholder="Bitiş yaşı (örn. 65)"
                value={newMaxAge}
                onChange={(e) => setNewMaxAge(e.target.value)}
                className="rounded-lg bg-slate-950 border border-slate-700 px-3 py-1.5 text-xs outline-none"
                disabled={newExpiryMode !== "age"}
              />

              <input
                type="text"
                placeholder="Kapsadığı kriterler (key, virgülle)"
                value={newCoversKeys}
                onChange={(e) => setNewCoversKeys(e.target.value)}
                className="rounded-lg bg-slate-950 border border-slate-700 px-3 py-1.5 text-xs outline-none"
              />
            </div>
            ...
            <div className="space-y-2">
              <label className="inline-flex items-center gap-2 text-xs text-slate-300">
                ...
                />
                Süre dolumu için uyarı üret
              </label>

              <div className="grid md:grid-cols-2 gap-3 text-xs text-slate-300">
                <select
                  disabled={!newTrackExpiry}
                  value={newWarnStart}
                  onChange={(e) => setNewWarnStart(e.target.value)}
                  className="rounded-lg bg-slate-950 border border-slate-700 px-3 py-1.5 outline-none"
                >
                  <option value="year:1">Bitişten 1 yıl önce</option>
                  <option value="month:6">Bitişten 6 ay önce</option>
                  <option value="month:3">Bitişten 3 ay önce</option>
                  <option value="month:1">Bitişten 1 ay önce</option>
                  <option value="day:15">Bitişten 15 gün önce</option>
                </select>

                <select
                  disabled={!newTrackExpiry}
                  value={newWarnInterval}
                  onChange={(e) => setNewWarnInterval(e.target.value)}
                  className="rounded-lg bg-slate-950 border border-slate-700 px-3 py-1.5 outline-none"
                >
                  <option value="month:1">Her ay</option>
                  <option value="month:3">3 ayda bir</option>
                  <option value="day:15">15 günde bir</option>
                  <option value="day:7">7 günde bir</option>
                </select>
              </div>
            </div>
            ...
            <button
              type="submit"
              className="rounded-lg bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-semibold text-sm px-4 py-1.5"
            >
              + Yeni Kriter Ekle
            </button>
          </form>
          ...
          <section className="bg-slate-900/70 border border-amber-500/60 rounded-xl p-4 space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-semibold text-amber-300">
                "{editingField.label}" kriterini düzenliyorsun
              </h2>
              <button
                type="button"
                onClick={cancelEdit}
            ...
            <form className="space-y-3" onSubmit={handleUpdate}>
              ...
              <div className="grid md:grid-cols-3 gap-3 items-center">
                <select
                  value={editOptionsGroupKey}
                  onChange={(e) => setEditOptionsGroupKey(e.target.value)}
                  className="rounded-lg bg-slate-950 border border-slate-700 px-3 py-1.5 text-xs outline-none"
                >
                  <option value="">Seçenek grubu yok (free text)</option>
                  {groups.map((g) => (
                ...
                <span className="text-[11px] text-slate-400 md:col-span-2">
                  İstersen kriterin bağlı olduğu grup motorunu burada
                  güncelleyebilirsin.
                </span>
              </div>
              ...
              <div className="grid md:grid-cols-3 gap-3 items-center">
                <select
                  ...
                  onChange={(e) =>
                    setEditExpiryMode(e.target.value as ExpiryMode)
                  }
                  className="rounded-lg bg-slate-950 border border-slate-700 px-3 py-1.5 text-xs outline-none"
                >
                  <option value="none">Süre / yaş kuralı yok</option>
                ...
                <input
                  ...
                  className="rounded-lg bg-slate-950 border border-slate-700 px-3 py-1.5 text-xs outline-none"
                  disabled={editExpiryMode !== "age"}
                />

                <input
                  type="text"
                  placeholder="Kapsadığı kriterler (key, virgülle)"
                  value={editCoversKeys}
                  onChange={(e) => setEditCoversKeys(e.target.value)}
                  className="rounded-lg bg-slate-950 border border-slate-700 px-3 py-1.5 text-xs outline-none"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid md:grid-cols-2 gap-3 text-xs text-slate-300">
                  <select
                    disabled={!editTrackExpiry}
                    value={editWarnStart}
                    onChange={(e) => setEditWarnStart(e.target.value)}
                    className="rounded-lg bg-slate-950 border border-slate-700 px-3 py-1.5 outline-none"
                  >
                    <option value="year:1">Bitişten 1 yıl önce</option>
                    <option value="month:6">Bitişten 6 ay önce</option>
                  ...
                  <select
                    disabled={!editTrackExpiry}
                    value={editWarnInterval}
                    onChange={(e) => setEditWarnInterval(e.target.value)}
                    className="rounded-lg bg-slate-950 border border-slate-700 px-3 py-1.5 outline-none"
                  >
                    <option value="month:1">Her ay</option>
                    <option value="month:3">3 ayda bir</option>
