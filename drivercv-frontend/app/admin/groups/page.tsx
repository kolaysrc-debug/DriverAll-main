...
function AdminFieldGroupsInner() {
  ...
  return (
    <div className="px-6 py-8 space-y-6">
      ...
      {/* ===== 3 SÜTUN: Gruplar | Nodes | Kurallar/Ayarlar ===== */}
      <div className="grid gap-4 lg:grid-cols-[1fr_1fr_1fr]">

        {/* ── SOL SÜTUN: Gruplar ── */}
        <div className="space-y-3">
          {/* Yeni Grup düğmesi + açılır form */}
          <button
            type="button"
            onClick={() => setShowNewGroupForm((p) => !p)}
            className="w-full rounded-md bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-500"
          >
            {showNewGroupForm ? "Kapat" : "+ Yeni Grup"}
          </button>
          ...
          {showNewGroupForm && (
            <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-3 space-y-2">
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-slate-400">Key (opsiyonel)</label>
                <input
                  className="w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-50 outline-none focus:border-sky-500"
              ...
              <button
                type="button"
                onClick={() => { handleCreateGroup(); setShowNewGroupForm(false); }}
                className="w-full rounded-md bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-500"
              >
                Oluştur
              </button>
          ...
          <section className="rounded-xl border border-slate-800 bg-slate-900/70 p-4 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-semibold text-slate-200">Gruplar</h2>
        ...
        <section className="rounded-xl border border-slate-800 bg-slate-900/80 p-4 shadow-sm">
          <h2 className="mb-3 text-sm font-semibold text-slate-200">Üyeler (Node'lar)</h2>
                ...
                <tbody>
                  {filteredSelectedNodes
                    ...
                    .map((n) => (
                      ...
                      >
                        <td className="px-3 py-2 text-xs font-mono text-slate-300">{n.key}</td>
                        <td className="px-3 py-2 text-sm text-slate-100">{n.label}</td>
        ...
        {/* ── SAĞ SÜTUN: Grup Düzenle VEYA Node Kuralları ── */}
        <section
          ref={editingGroup ? editGroupPanelRef : editNodePanelRef}
          className="rounded-xl border border-slate-800 bg-slate-900/80 p-4 shadow-sm"
        >
          {editingGroup ? (
            <>
              <h2 className="mb-3 text-sm font-semibold text-slate-200">
                Grup Ayarları: {editingGroup.groupLabel}
              </h2>
              ...
              <div className="space-y-3">
                ...
                <div>
                  <label className="mb-1 block text-xs font-medium text-slate-400">Ülke</label>
                  <input
                    className="w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-50 outline-none focus:border-sky-500"
                ...
                <div>
                  <label className="mb-1 block text-xs font-medium text-slate-400">Geçerlilik modeli</label>
                  <select
                    className="w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-50 outline-none focus:border-sky-500"
                    value={editingGroup.validityModel || "none"}
                    onChange={(e) => setEditingGroup({ ...editingGroup, validityModel: e.target.value })}
                  >
                ...
                <div>
                  <label className="mb-1 block text-xs font-medium text-slate-400">Seçim modu</label>
                  <select
                    className="w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-50 outline-none focus:border-sky-500"
                ...
                <div>
                  <label className="mb-1 block text-xs font-medium text-slate-400">Geçerlilik süresi (yıl)</label>
                  <input
                    type="number"
                    className="w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-50 outline-none focus:border-sky-500"
                    ...
                    onChange={(e) =>
                      setEditingGroup({ ...editingGroup, maxAge: e.target.value ? Number(e.target.value) : (null as any) })
                    }
                  />
                </div>
            ...
            <>
              <h2 className="mb-3 text-sm font-semibold text-slate-200">Kurallar</h2>
                ...
                <div className="rounded-lg border border-slate-800 bg-slate-950/60 p-4">
                  {!editingNode ? (
                    <div className="text-sm text-slate-300">
                      Ortadan bir node seçerek kurallarını düzenleyebilirsin.
                    </div>
                  ) : (
                    <>
                      ...
                      <div className="grid gap-3 md:grid-cols-2">
                        <div>
                          ...
                          <input className="w-full rounded-md border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-300" value={editingNodeForm.key} disabled />
                        </div>
                        <div>
                          <label className="mb-1 block text-xs font-medium text-slate-400">Etiket</label>
                          <input className="w-full rounded-md border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-300" value={editingNodeForm.label} disabled />
                        </div>
                      </div>

                      <div className="mt-3 grid gap-3 md:grid-cols-2">
                        <div>
                          <label className="mb-1 block text-xs font-medium text-slate-400">Üst üye (parent)</label>
                          <select
                            className="w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-50 outline-none focus:border-sky-500"
                            value={editingNodeForm.parentKey}
                            onChange={(e) => setEditingNodeForm((prev) => ({ ...prev, parentKey: e.target.value }))}
                          >
                        ...
                        <div>
                          <label className="mb-1 block text-xs font-medium text-slate-400">Sıra (sort)</label>
                          <input
                            type="number"
                            className="w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-50 outline-none focus:border-sky-500"
                            placeholder="0"
                            value={editingNodeForm.sortOrder}
                            onChange={(e) => setEditingNodeForm((prev) => ({ ...prev, sortOrder: e.target.value }))}
                          />
                        </div>
                      </div>
                      ...
                      <div className="mt-3 grid gap-3 md:grid-cols-2">
                        <div>
                          <label className="mb-1 block text-xs font-medium text-slate-400">Kapsar (coverage)</label>
                          <div className="h-36 overflow-auto rounded-md border border-slate-700 bg-slate-950 px-3 py-2">
                            {selectedGroupNodeKeys
                              .filter((k) => k !== editingNodeForm.key)
                              .map((k) => (
                                <label key={k} className="flex items-center gap-2 py-1 text-xs text-slate-100">
                                  <input
                                    type="checkbox"
                                    className="h-4 w-4 rounded border-slate-600 bg-slate-900"
                                    checked={editingNodeForm.coverage.includes(k)}
                                    onChange={() => setEditingNodeForm((prev) => ({ ...prev, coverage: toggleArrayValue(prev.coverage, k) }))}
                                  />
                        ...
                        <div>
                          <label className="mb-1 block text-xs font-medium text-slate-400">Olmadan olmaz (requiredWith)</label>
                          <div className="h-36 overflow-auto rounded-md border border-slate-700 bg-slate-950 px-3 py-2">
                            {selectedGroupNodeKeys
                              .filter((k) => k !== editingNodeForm.key)
                              .map((k) => (
                                <label key={k} className="flex items-center gap-2 py-1 text-xs text-slate-100">
                                  <input
                                    type="checkbox"
                                    className="h-4 w-4 rounded border-slate-600 bg-slate-900"
                                    checked={editingNodeForm.requiredWith.includes(k)}
                                    onChange={() =>
                                      setEditingNodeForm((prev) => ({
                                        ...prev,
                                        requiredWith: enforceAdrPrereq(prev.key, toggleArrayValue(prev.requiredWith, k)),
                                      }))
                                    }
                                  />
                                  <span className="font-mono text-[11px] text-slate-300">{k}</span>
                      ...
                      <div className="mt-3">
                        <label className="mb-1 block text-xs font-medium text-slate-400">Eşdeğer anahtarlar (equivalentKeys)</label>
                        <div className="mb-2 flex items-center justify-end gap-2">
                          <button
                            ...
                            onClick={() => setEditingNodeForm((prev) => ({ ...prev, equivalentKeys: [] }))}
                          >
                            Hepsini kaldır
                          </button>
                          <button
                            ...
                            onClick={() =>
                              setEditingNodeForm((prev) => ({
                                ...prev,
                                equivalentKeys: selectedGroupNodeKeys.filter((k) => k !== prev.key),
                              }))
                            }
                          >
                            Hepsini seç
                            ...
                            .map((k) => (
                              <label key={k} className="flex items-center gap-2 py-1 text-xs text-slate-100">
                                <input
                                  type="checkbox"
                                  className="h-4 w-4 rounded border-slate-600 bg-slate-900"
                                  checked={editingNodeForm.equivalentKeys.includes(k)}
                                  onChange={() => setEditingNodeForm((prev) => ({ ...prev, equivalentKeys: toggleArrayValue(prev.equivalentKeys, k) }))}
                                />
                                <span className="font-mono text-[11px] text-slate-300">{k}</span>
                              </label>
                            ))}
                      ...
                      <div className="mt-3 flex items-center justify-between gap-3">
                        <label className="flex items-center gap-2 text-xs font-medium text-slate-300">
                          <input
                            ...
                            onChange={(e) => setEditingNodeForm((prev) => ({ ...prev, active: e.target.checked }))}
                          />
                          Aktif
                        </label>
