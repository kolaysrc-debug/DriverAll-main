...
export default function AdminJobApprovalsPage() {
  ...
  async function doReject(id: string) {
    ...
    setErr(null);
    try {
      const adminNote = window.prompt("Red notu (opsiyonel)", "") || "";
      await rejectJob(id, { adminNote });
      await load();
    } catch (e: any) {
      if (isAuthError(e)) {
        handleAuthFailure("Oturum geçersiz (HTTP 401/403)");
                  ...
                  <div key={j._id} className="bg-slate-950/50 border border-slate-800 rounded-lg p-3">
                    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <button
                          type="button"
                          onClick={() => doReject(j._id)}
                          disabled={disabled}
                          className="text-xs px-3 py-2 rounded-lg bg-rose-500 text-slate-950 font-semibold hover:bg-rose-400 disabled:opacity-60 disabled:cursor-not-allowed"
                        >
                          Reddet
                        </button>
